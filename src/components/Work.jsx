        ...
        <div className="w-full mt-2">
            <div className="relative max-w-screen-lg mx-auto text-center">
                ...
                <div className="absolute top-0 w-full h-full">
                    {img.map((item , index) => item.isActive && (
                        <img className="absolute w-52 rounded-lg -translate-x-[50%] -translate-y-[50%]" style={{top:item.top , left:item.left}} src={item.url} alt="" key={index}></img>
                    ))}
                </div>
            </div>
        </div>
